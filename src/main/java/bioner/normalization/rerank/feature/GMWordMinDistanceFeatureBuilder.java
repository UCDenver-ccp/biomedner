...
public class GMWordMinDistanceFeatureBuilder implements RerankFeatureBuilder {

	private Pattern m_pattern;
	private int m_beginBound = Integer.MAX_VALUE;
	private int m_endBound = Integer.MAX_VALUE;
	public GMWordMinDistanceFeatureBuilder(String patternStr)
	{
		m_pattern = Pattern.compile(patternStr);
	}
	public GMWordMinDistanceFeatureBuilder(String patternStr, int beginBound, int endBound)
	{
		m_pattern = Pattern.compile(patternStr);
	...
	public String extractFeature(BioNERDocument document,
		...
		if(scoreMin<0.0999) return "0.0";
		return Double.toString(scoreMin);
	}
